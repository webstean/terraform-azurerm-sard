locals {
  ...
  cert_random_suffix = substr(random_string.environment.result, 0, 6)
  cert_name_hostname = lower(substr(replace("c${local.cert_random_suffix}${local.cert_name_location}", "-", ""), 0, 24))
