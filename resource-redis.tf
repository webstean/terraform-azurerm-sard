locals {
  ...
  redis_docs          = "https://learn.microsoft.com/en-us/azure/redis/"
  redis_name          = "redis-${var.prefix}"
  redis_name_location = lower("${local.redis_name}-${lower(var.location)}")
  redis_random_suffix = substr(random_string.environment.result, 0, 6)
  redis_name_hostname = lower(substr(replace("r${local.redis_random_suffix}${local.redis_name_location}", "-", ""), 0, 24))
  ...
  ## Single endpoint, automatic sharding (default) : "OSSCluster"
  ## Redis Cluster API protocol, best performance  : "NoEviction"
  ## Non-clustered mode, maximum 25GB Default      : "EnterpriseCluster"
}
...
output "redis_name" {
  description = <<DESC
The Name of this environment's Redis instance.
DESC
  sensitive   = false
...
output "redis_resource" {
  description = <<DESC
Details of this environment's Redis instance.
DESC
  sensitive   = false
...
output "redis_default_database" {
  description = <<DESC
The Name of the default database within this environment's Redis instance
DESC
  sensitive   = false
