
locals {
  vwan_friendly_name          = "Virtual WAN"
  vwan_hub_name               = "vwan-${var.prefix}"
  vwan_hub_name_location      = lower("${local.vwan_hub_name}-${lower(var.location)}")
  vwan_hub_random_suffix      = substr(md5(local.vwan_hub_name_location), 0, 6)
  vwan_hub_name_hostname      = lower(substr(replace("l${local.vwan_hub_random_suffix}${local.vwan_hub_name_location}", "-", ""), 0, 24))
  vwan_hub_sku                = "Basic"      ## Possible values include: Basic and Standard. Defaults to Standard.
  vwan_hub_routing_preference = "VpnGateway" ## Possible values include: ExpressRoute, ASPath and VpnGateway.
}

## $$ (unles vWAN is basic SKU) We need 1x vWAN Hub in each region (take a long time to initially deploy - 30+ minutes)
resource "azurerm_virtual_hub" "this" {
  count = var.virtual_wan_hub_id == null ? 1 : 0

  name                = local.vwan_hub_name
  resource_group_name = module.environment_resource_group.resource.name
  location            = module.environment_resource_group.resource.location

  virtual_wan_id = var.virtual_wan_id

  ## address space
  address_prefix = "10.99.${local.regions[module.environment_resource_group.resource.location].location_number}.0/24"
  sku            = local.vwan_hub_sku ## Possible values are Basic and Standard.

  hub_routing_preference = local.vwan_hub_routing_preference

  tags = { for key, value in module.environment_resource_group.resource.tags : key => value if lower(key) != "created" }
}

resource "azurerm_virtual_hub_connection" "vnet" {
  name                      = "link-VNET:${azurerm_virtual_network.this.name}-to-HUB:${azurerm_virtual_hub.this[0].name}"
  virtual_hub_id            = try(azurerm_virtual_hub.this[0].id, var.virtual_wan_hub_id)
  remote_virtual_network_id = azurerm_virtual_network.this.id
}

output "virtual_wan_hub_id" {
  description = "The ID of the vWAN Hub."
  sensitive   = false
  value       = try(azurerm_virtual_hub.this[0].id, var.virtual_wan_hub_id)
}

/*
resource "azurerm_vpn_gateway" "this" {
  count = local.vwan_hub_sku == "Standard" ? 1 : 0

  ## we only need one, since the Hubs are peered together (with Standard SKU, not Basic!)

  name                                  = "vpn-gateway-${var.prefix}-${title(each.value.short_name)}"
  resource_group_name                   = module.environment_resource_group.resource.name
  location                              = module.environment_resource_group.resource.location
  virtual_hub_id                        = try(azurerm_virtual_hub.this[0].id, var.vwan_hub_id)
  bgp_route_translation_for_nat_enabled = false
  routing_preference                    = "Microsoft Network"
  scale_unit                            = 1

  tags = { for key, value in module.environment_resource_group.resource.tags : key => value if lower(key) != "created" }
}
*/
